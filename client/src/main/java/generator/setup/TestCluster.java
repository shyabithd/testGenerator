...
import generator.utils.ListUtil;
...
public class TestCluster {
	...
	public void removeUnusableGenerators(){
		...
		for(Entry<GenericClass,Set<GenericAccessibleObject<?>>> entry : generators.entrySet()){
	...
	public void invalidateGeneratorCache(GenericClass klass){
		Iterator<Entry<GenericClass,Set<GenericAccessibleObject<?>>>> iter = generatorCache.entrySet().iterator();
		while(iter.hasNext()){
...
//			CastClassManager.getInstance().addCastClass(clazz, 1);
			clearGeneratorCache(new GenericClass((ClassReader) null));
		}
	}
