...
import generator.setup.callgraph.CallGraphGenerator;
...
public class DependencyAnalysis {
	...
	private static Map<String, ClassReader> classCache = new LinkedHashMap<String, ClassReader>();
...
//		}
		...
		if (ArrayUtil.contains(Properties.CRITERION, Properties.Criterion.IBRANCH)
	...
	public static Set<String> analyzeTarget(String target, List<String> classPath) throws RuntimeException,
		...
		targetClasses = ResourceList.getInstance(TestGenerationContext.getInstance().getClassReader().getClass().getClassLoader()).getAllClasses(target, false);
...
//	}
	...
	public static ClassReader getClassNode(String className) {
	...
	public static Collection<ClassReader> getAllClassNodes() {
		return classCache.values();
	}

	private static ClassReader loadClassNode(String className) throws IOException {
		
		InputStream classStream = ResourceList.getInstance(null).getClassAsStream(className);
		if(classStream == null) {
	...
	private static void gatherStatistics() {
		ClientServices.getInstance().getClientNode()
				.trackOutputVariable(RuntimeVariable.Predicates, BranchPool.getInstance(TestGenerationContext.getInstance().getClassReader()).getBranchCounter());
		ClientServices.getInstance().getClientNode()
				.trackOutputVariable(RuntimeVariable.Instrumented_Predicates, BranchPool.getInstance(TestGenerationContext.getInstance().getClassReader()).getNumArtificialBranches());
		int numBranches = BranchPool.getInstance(TestGenerationContext.getInstance().getClassReader()).getBranchCounter() * 2;
		ClientServices
				.getInstance()
				.getClientNode()
		...
		ClientServices
				...
				.trackOutputVariable(RuntimeVariable.Total_Branches_Real,
						((BranchPool.getInstance(TestGenerationContext.getInstance().getClassReader()).getBranchCounter() - BranchPool.getInstance(TestGenerationContext.getInstance().getClassReader()).getNumArtificialBranches()))/2);
		ClientServices
				.getInstance()
				.getClientNode()
				.trackOutputVariable(RuntimeVariable.Total_Branches_Instrumented,
						(BranchPool.getInstance(TestGenerationContext.getInstance().getClassReader()).getNumArtificialBranches()));
		ClientServices
				.getInstance()
				.getClientNode()
				.trackOutputVariable(RuntimeVariable.Branchless_Methods,
						BranchPool.getInstance(TestGenerationContext.getInstance().getClassReader()).getBranchlessMethods().size());
//		ClientServices
//				.getInstance()
//				.getClientNode()
//				.trackOutputVariable(RuntimeVariable.Total_Methods,
//						CFGMethodAdapter.getNumMethods(TestGenerationContext.getInstance().getClassReader()));
