	...
	public static InheritanceTree createFromClassPath(List<String> classPath) {
		if (!Properties.INSTRUMENT_CONTEXT && !Properties.INHERITANCE_FILE.isEmpty()) {
			try {
				...
				LoggingUtils.getGeneratorLogger().info("* Inheritance tree loaded from {}",
				                                 Properties.INHERITANCE_FILE);
				return tree;

			} catch (IOException e) {
				LoggingUtils.getGeneratorLogger().warn("* Error loading inheritance tree: {}", e);
			}
		}
		...
		for (String classPathEntry : classPath) {
			...
			LoggingUtils.getGeneratorLogger().info("  - " + classPathEntry);
...
//		analyzeClassStream(inheritanceTree, stream, false);
	...
	private static void analyzeClassStream(InheritanceTree inheritanceTree,
			...
			analyzeClassNode(inheritanceTree, classReader, onlyPublic);
	}
	
	@SuppressWarnings("unchecked")
	private static void analyzeClassNode(InheritanceTree inheritanceTree,
										 ClassReader cn, boolean onlyPublic) {
		
		logger.info("Analyzing class {}", cn.getClassName());
		
		// Don't load classes already seen from a different CP entry
		if(inheritanceTree.hasClass(cn.getClassName()))
			return;
...
//		}
	...
	public static boolean canUse(ClassReader cn) {

//		if ((cn.access & Opcodes.ACC_PRIVATE) == Opcodes.ACC_PRIVATE) {
//			logger.debug(cn.name + " is private, ignoring it");
//			return false;
//		}
		if (ANONYMOUS_MATCHER1.matcher(cn.getClassName()).matches()) {
			logger.debug(cn.getClassName() + " looks like an anonymous class, ignoring it");
			return false;
		}

		if (ANONYMOUS_MATCHER2.matcher(cn.getClassName()).matches()) {
			logger.debug(cn.getClassName() + " looks like an anonymous class, ignoring it");
			return false;
		}
		...
		if (cn.getClassName().startsWith("junit"))
			return false;
...
//		}
		...
		logger.debug(cn.getClassName() + " is accessible");
		return true;
	}
