...
import generator.contracts.ContractViolation;
		...
		for (Statement s : statements) {
			for (VariableReference var : s.getVariableReferences()) {
				if (var != null && !var.isPrimitive()) {
					Class clazz = var.getVariableClass().getClass();
...
//						clazz = clazz.getComponentType();
	...
	public List<VariableReference> getObjects(DataType type, int position) {
		...
		Class<?> rawClass = genericClass.getRawClass().getClass();
		for (int i = 0; i < position && i < size(); i++) {
			...
			if (value instanceof ArrayReference) {
				...
				if (value.isArray()) {
					Class<?> arrayClass = value.getVariableClass().getClass();
					isClassUtilsBug = isClassUtilsBug(rawClass, arrayClass);
				}
				if (rawClass.isArray() && !isClassUtilsBug) {
					isClassUtilsBug = isClassUtilsBug(value.getVariableClass().getClass(), rawClass);
				}
	...
	public void removeDownCasts() {
		for(Statement s : statements) {
			if(s instanceof MethodStatement) {
				...
				Class<?> variableClass = retVal.getVariableClass().getClass();
