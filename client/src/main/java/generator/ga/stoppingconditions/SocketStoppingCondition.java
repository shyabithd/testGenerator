 ...
 * 
 */
package generator.ga.stoppingconditions;
		...
		Thread t = new Thread() {
			...
			public void run() {
				...
				try {
					...
					serverSocket.accept();
					LoggingUtils.getGeneratorLogger().info("* Stopping request received");
					interrupted = true;

				} catch (IOException e) {
					LoggingUtils.getGeneratorLogger().warn("Failed to create socket on port "
